...
As is visible, this change improved the rate of improvement drastically. In general, it is best to try to make gradual improvements as easy as possible, as it typically leads to good results.

=== Increasing
The goal of this task was to produce sequences of increasing numbers. Fitness was measured by how many differences between sequential outputs were positive. The hope was that the algorithm would learn to use the operation instruction to repeatedly add to a number, thereby generating an increasing sequence with minimal effort. However, instead, it just hard-coded increasing sequences, followed by a jump back to the top to get the highest fitness possible  (see @hardcoded-increasing). I expected this when creating this task, but had some hope that the algorithm might figure out the "better" way of doing it. I guess my hope was misplaced.
...
=== Add Eight
The goal of this task is similar to `cat`, except every output should be the input, plus eight, and it was scored similarly. The reason that the task was specifically "add 8" and not "increment" was that by having the number not be _directly next to_ the input, it gave programs more leniency; for example, if it was just "increment", then the program that adds two to its input and the program that just directly outputs its input would give the same fitness, when the former is much closer to what we want. With a higher number, such as eight, there's now a whole fifteen different options that give higher fitness than doing nothing to the input (adding the seven numbers before eight, adding eight itself, and adding the seven numbers after eight). As discussed previously, this extra leniency is almost always a good thing. At first, the algorithm exploited a bug in my code to gain fitness infinitely, which I quickly fixed (this is another instance of specification gaming). Once I fixed that, it managed the task decently well.

Something of note is that one program, instead of adding eight, instead subtracted 247, which under arithmetic mod 256 (which is used since the general purpose registers are eight-bit), is the same operation as adding eight. This demonstrates that genetic algorithms sometimes give unexpected results.

== Mutation Rate and Population Size
Experimentally, a 100% mutation rate appears optimal. In the statistics previously shown, the mutation rate used was 100%, due to that. The reasons for this, I believe, are that #inline-num(1) it helps explore the possibility space in less time, and #inline-num(2) there exists some what I call a _soft mutation rate_; this is the amount of mutations that actually have an effect on the performance of the program #sym.dash.em some programs have a number of "garbage" instructions in them (instructions that don't do anything, see @garbage-instructions), so the effects of a high mutation rate (especially a rate of 100%, which would presumably have bad effects since beneficial mutations could be instantly mutated away) appears to be limited. However, a low mutation rate appears to be generally worse, since it just seems to delay the evolution of programs.
...
A out 111
  ...
  out 33
	```,
	caption: [A hello world program generated by the algorithm. Many instructions here are "garbage" and don't do or output anything.]
) <garbage-instructions>
The effects of population size do have a significant effect though, in general higher population sizes increase the rate at which the algorithm improves. This makes intuitive sense; the more individuals, the faster the possibility space can be explored. However, higher population sizes also result in more computational cost, so for all trials in @tasks, a size of 1000 was used; this choice was mostly arbitrary, but it appeared to be a decent size that wasn't too big nor too small.
...
As previously mentioned, there is a problem of programs having many "garbage" instructions that clutter up the program. In some cases these can slow down the program, leading to the instructions being optimized out naturally, but in most cases these are innert #sym.dash.em having no effect on fitness. However, these extraneous instructions are still somewhat undesirable. To counter this, I tried adding a term to the fitness function to disincentivize longer programs (which is a _genotypic_ requirement, as introduced in @ffd-section) to mixed results. For one, the term must be insignificant enough that it doesn't hinder progress (a program that does the task better but is longer should still have higher fitness than one that's shorter but worse at the task) but significant enough that when the space is competitive, it favors shorter programs. Having every instruction subtract a tenth of a point (or more accurately, multiplying the fitness by ten and then subtracting one point for each instruction used) seemed to work well. However, for some tasks, this meant that the empty program, while having a lower fitness than a program which does the task correctly, would take less effort to get to than a correct program, and therefore be reached quicker; at which point the algorithm would reach a local maximum and be unable to improve.

= Discussion
== Limitations <limitations>
There are a few limitations in my approach #sym.dash.em primarily, it can not do very complex tasks. The selection shown here is mostly simple tasks that would take any half-decent programmer a few minutes at most, and the programs that they would produce would be of a much higher quality. Trying to give it anything remotely complex will result in failure, since a program that does even remotely what one wants it to do is much more complicated than this algorithm will reach in a reasonable amount of time. This is also the reason there's no conditional instructions in the architecture #sym.dash.em any task that would require the use of those would be too complicated for the algorithm (in its current form) to accomplish.

Another limitation is that genetic algorithms like this one tend to require a lot of computation time, since it has to simulate a whole population every single generation, and most mutations done will tend not to be beneficial regardless. There's a reason gradient descent is as widely used as it is #sym.dash.em it's simply a much more efficient use of computational resources.
