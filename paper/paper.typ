...
As the char demonstrates, the change to the Levenshtein distance function drastically improved the rate of fitness gain. In general, it is best to try to make gradual improvements as easy as possible, as it typically leads to good results.
...
=== Increasing
The goal of this task was to produce sequences of increasing numbers. Fitness was measured by how many differences between sequential outputs were positive. The desired outcome was that the algorithm would learn to use the operation instruction to repeatedly add to a number, thereby generating an increasing sequence with minimal effort. However, instead, the algorithm generated a "lazier" program; it created hard-coded increasing sequences, followed by a jump back to the top to get the highest fitness possible  (see @hardcoded-increasing). This was expected, since the algorithm tends towards less "intelligent", but simpler, programs.
...
Another limitation is that genetic algorithms like this one tend to require a lot of computation time, since it has to simulate a whole population every single generation, and most mutations done will tend not to be beneficial regardless.
